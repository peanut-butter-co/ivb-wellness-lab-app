...
function isValidBundle(cart, bundle, performTaxExemption) {
	...
	const packConfigStr = parent.merchandise.product.packs_config?.value;
	if (!packConfigStr) {
		return null;
	}

	// Parse or retrieve from cache
	const productId = parent.merchandise.product.id;
	let packsConfig = packConfigCache.get(productId);
	if (!packsConfig) {
		try {
			packsConfig = JSON.parse(packConfigStr);
			packConfigCache.set(productId, packsConfig);
		} catch {
			return null;
		}
	}

	if (!packsConfig || !packsConfig.items || packsConfig.items.length === 0) {
		return null;
	}

	const parentCost = parseFloat(parent.cost.amountPerQuantity.amount || "0");
	let packTotal = parentCost;

	for (const line of childLines) {
		const linePrice = parseFloat(line.cost.amountPerQuantity.amount || "0");
		packTotal += linePrice;
	}
